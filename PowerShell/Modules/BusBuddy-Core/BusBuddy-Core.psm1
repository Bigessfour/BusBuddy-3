...
function Test-BusBuddySqlConnection {
    ...
    try {
        ...
        if (-not $localDbInstances) {
            ...
            $localDbResult = Invoke-Sqlcmd -ServerInstance "(localdb)\MSSQLLocalDB" -Database "BusBuddy" -Query "SELECT @@VERSION AS Version" -ErrorAction Stop
            Write-Information "✅ LocalDB connection successful" -InformationAction Continue
            Write-Information "   LocalDB Version: $($localDbResult.Version)" -InformationAction Continue
        }
    }
    catch {
...
<#
.SYNOPSIS
Get the BusBuddy workspace root directory

.DESCRIPTION
Determines the BusBuddy workspace root directory by checking the current working directory
or falling back to the default BusBuddy installation path.

    .EXAMPLE
$root = Get-BusBuddyWorkspaceRoot
Write-Host "Workspace root: $root"

.NOTES
This function is used internally by BusBuddy modules to locate the workspace root.
#>
...
function Invoke-BusBuddyBuild {
    ...
    try {
        ...
        $env:BUSBUDDY_NO_WELCOME = $env:BUSBUDDY_NO_WELCOME ?? '1'
        $env:BUSBUDDY_NO_XAI_WARN = $env:BUSBUDDY_NO_XAI_WARN ?? '1'
        $env:BUSBUDDY_SILENT = $env:BUSBUDDY_SILENT ?? '1'
