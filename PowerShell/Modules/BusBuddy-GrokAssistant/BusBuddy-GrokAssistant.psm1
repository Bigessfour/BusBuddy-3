...
$grokConfigPath = Join-Path $PSScriptRoot '..\grok-config\grok-config.psm1'
Import-Module $grokConfigPath -Force -ErrorAction Stop
...
function Invoke-GrokApi {
    ...
    param (
        ...
        [string]$Model = $GrokConfig.DefaultModel,
    ...
    if ($GrokConfig.UseLiveAPI -and $global:GrokModuleState.ResponseCache.ContainsKey($cacheKey)) {
    ...
    # Retry logic
    while ($attempt -lt $GrokConfig.RetryAttempts) {
        try {
            Write-Verbose "API attempt $($attempt + 1)/$($GrokConfig.RetryAttempts)"
            $response = Invoke-RestMethod -Uri "$($GrokConfig.BaseUrl)/chat/completions" -Method Post -Headers $headers -Body $body -TimeoutSec $GrokConfig.TimeoutSeconds
            break
        } catch {
            $lastError = $_
            ...
            elseif ($_.Exception.Response.StatusCode -eq 429) {
                ...
                Start-Sleep -Seconds ($GrokConfig.RetryDelaySeconds * 2)
            }

            $attempt++
            if ($attempt -eq $GrokConfig.RetryAttempts) {
                throw "API call failed after $attempt attempts: $($_.Exception.Message)"
            }
            Write-Warning "API attempt $attempt failed, retrying in $($GrokConfig.RetryDelaySeconds) seconds..."
            Start-Sleep -Seconds $GrokConfig.RetryDelaySeconds
        }
    }
...
function Test-GrokConnection {
    ...
    try {
        ...
        if ($response -like "*successful*") {
            ...
            Write-Output "   Model: $($GrokConfig.DefaultModel)"
            Write-Output "   Base URL: $($GrokConfig.BaseUrl)"
...
function Get-GrokConfig {
    ...
    $configCopy = $GrokConfig.Clone()
    if ($configCopy.ApiKey) {
        $configCopy.ApiKey = "***MASKED*** (Length: $($GrokConfig.ApiKey.Length))"
    } else {
...
function Invoke-GrokCIAnalysis {
    ...
    if (-not $GrokConfig.EnableCIAnalysis) {
        throw "CI Analysis feature is disabled in configuration. Enable it with: `$GrokConfig.EnableCIAnalysis = `$true"
    }
    ...
    $prompt = $GrokConfig.Prompts.CIFailureAnalysis -f $fullDetails
...
function Invoke-GrokRouteOptimization {
    ...
    if (-not $GrokConfig.EnableRouteOptimization) {
...
"@
    ...
    $prompt = $GrokConfig.Prompts.RouteOptimization -f $fullDetails
...
function Invoke-GrokMaintenancePrediction {
    ...
    if (-not $GrokConfig.EnableMaintenancePrediction) {
...
"@
    ...
    $prompt = $GrokConfig.Prompts.MaintenancePrediction -f $fullDetails
...
function Invoke-GrokCodeReview {
    ...
    if (-not $GrokConfig.EnableCodeReview) {
    ...
    $prompt = $GrokConfig.Prompts.CodeReview -f $Code
    return Invoke-GrokApi -Prompt $prompt
}
...
function Invoke-GrokPerformanceAnalysis {
    ...
    if (-not $GrokConfig.EnablePerformanceAnalysis) {
    ...
    $prompt = $GrokConfig.Prompts.PerformanceAnalysis -f $PerformanceData
    return Invoke-GrokApi -Prompt $prompt
}
...
function Invoke-GrokSecurityAudit {
    ...
    if (-not $GrokConfig.EnableSecurityAudit) {
    ...
    $prompt = $GrokConfig.Prompts.SecurityAudit -f $SecurityContext
    return Invoke-GrokApi -Prompt $prompt
}
...
function Invoke-GrokArchitectureReview {
    ...
    if (-not $GrokConfig.EnableArchitectureReview) {
    ...
    $prompt = $GrokConfig.Prompts.ArchitectureReview -f $ArchitectureDetails
    return Invoke-GrokApi -Prompt $prompt
}
...
function Invoke-GrokTestOptimization {
    ...
    if (-not $GrokConfig.EnableTestOptimization) {
    ...
    $prompt = $GrokConfig.Prompts.TestOptimization -f $TestContext
    return Invoke-GrokApi -Prompt $prompt
}
...
function Invoke-GrokDependencyAnalysis {
    ...
    if (-not $GrokConfig.EnableDependencyAnalysis) {
    ...
    $prompt = $GrokConfig.Prompts.DependencyAnalysis -f $DependencyData
    return Invoke-GrokApi -Prompt $prompt
}
...
function Invoke-GrokDatabaseOptimization {
    ...
    $prompt = $GrokConfig.Prompts.DatabaseOptimization -f $DatabaseContext
    return Invoke-GrokApi -Prompt $prompt
}
...
function Invoke-GrokUIUXReview {
    ...
    $prompt = $GrokConfig.Prompts.UIUXReview -f $UIUXContext
    return Invoke-GrokApi -Prompt $prompt
}
