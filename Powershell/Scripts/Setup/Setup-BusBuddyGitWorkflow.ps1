...
# Rename non-approved verb function bb-* -> Set-BusBuddyGitWorkflow (preview shown)
# Old:
...
${5:General notes}
#>
function Set-BusBuddyGitWorkflow {
    [CmdletBinding(SupportsShouldProcess)]
    param(
        [string]$Repo = '.'
    )

    if ($PSCmdlet.ShouldProcess("BusBuddy GitHub Workflow in $Repo", "Configure")) {
        # ...existing implementation...
        Write-Information "Configuring BusBuddy GitHub workflow in $Repo" -InformationAction Continue
    }
    else {
        Write-Information "❌ Workflow configuration cancelled by user" -InformationAction Continue
        return $false
    }
}
# If an alias existed, update or remove it; prefer explicit alias creation:
# # Set-Alias -Name bb -Value Set-BusBuddyGitWorkflow  # avoid if we want only approved verbs
