...
- Update _all_ references (README, copilot, docs, CI, PS validators) in one pass.
...
- BusBuddy.Tests/\*_/_ (add/ensure coverage for every provable item).
- README.md, .github/copilot-instructions.md, Documentation/_-MANAGEMENT_.md, DEVELOPMENT-GUIDE.md, etc. (version/docs sync).
...
- Hygiene targets: root clutter files, flat ViewModels/_.cs, Phase_/MVP comments, ci-with-ai skew (or deprecate ai one), etc.
...
- Purged/moved many unneeded or no-longer-participating PS files to `Documentation/Archive/PowerShell-Legacy/` (root fix-_.ps1, test-_.ps1, generate-fetchability\*.ps1, legacy cleanup scripts, empty/stub modules like BusBuddy-Development.psd1/.psm1 and dups, etc.).
...
- **BusBuddy.Tests now cover each item that needs to be proven** (per request):
    ...
    - **Existing + reused for full list coverage** (no gaps):
        ...
        - Package update "proven": BusBuddy.Tests.csproj (and Core) successfully builds and runs tests against the _updated_ centralized Syncfusion 33.2.10 + other packages via props (no breakage; CI will gate it). The test project references Syncfusion explicitly for "UI testing" coverage.
...
- Historical/non-production docs: CONSOLIDATION-PLAN, Legacy-Cleanup-_, Route-Foundation-Assessment, UAT-Plan-Excellence, VALIDATION-COMPLETE-_, Student-Entry-Route-Design-Guide-Complete, Examples/, stray Reports/\*.json.
...
_Map generated 2026-06-15. Re-validate with mermaid-mcp after major structural changes._
