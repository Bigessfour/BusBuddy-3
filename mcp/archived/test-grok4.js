...
const PROJECT_ROOT =
    process.env.BUSBUDDY_PROJECT_ROOT || path.resolve(process.cwd(), "..");
...
async function testGrok4Server() {
    ...
    try {
        ...
        if (
            stderr.includes("XAI_API_KEY not set") &&
            stderr.includes("Grok-4 MCP server running")
        ) {
