...
class GitMCPServer {
  ...
  setupErrorHandling() {
    this.server.onerror = (error) => console.error("[Git MCP Error]", error);
      ...
      try {
        switch (name) {
          ...
          case "git-log":
            ...
            return await this.runGitCommand(["log", "--oneline", `-${count}`]);
          
          case "git-diff":
            const diffArgs = args?.file ? ["diff", args.file] : ["diff"];
            return await this.runGitCommand(diffArgs);
          
          case "git-branch":
            const branchArgs = args?.remote ? ["branch", "-a"] : ["branch"];
            return await this.runGitCommand(branchArgs);
          
          case "git-add":
            return await this.runGitCommand(["add", ...args.files]);
          
          case "git-commit":
            return await this.runGitCommand(["commit", "-m", args.message]);
