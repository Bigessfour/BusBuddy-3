        ...
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            project: {
                                type: "string",
                                description:
                                    "Specific test project to run (optional)",
                            },
                        },
                    },
                },
                {
                    ...
                    description:
                        "Execute a safe SQL query against the BusBuddy database",
                    inputSchema: {
                        type: "object",
                        properties: {
                            query: {
                            ...
                            table: {
                                type: "string",
                                description:
                                    "Table name for simple queries (Students, Drivers, Vehicles, Routes)",
                            },
                        },
                        required: ["query"],
                    },
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            level: {
                                type: "string",
                                description:
                                    "Log level filter (Debug, Information, Warning, Error)",
                                default: "Information",
                            },
                        },
                    },
                },
                {
                    ...
                    description:
                        "Get comprehensive BusBuddy project status and metrics",
                    inputSchema: {
                        type: "object",
                        properties: {},
                    },
                },
                {
                    ...
                    description:
                        "Verify Syncfusion WPF controls and licensing status",
                    inputSchema: {
                        type: "object",
                        properties: {},
                    },
            ...
            async (request) => {
                ...
                try {
                    switch (name) {
                        ...
                        case "busbuddy-logs":
                            return await this.getBusBuddyLogs(
                                args?.lines || 50,
                                args?.level || "Information",
                            );
    ...
    async getProjectStatus() {
        try {
            ...
            const solutionFile = await fs.readFile(
                path.join(PROJECT_ROOT, "BusBuddy.sln"),
                "utf-8",
            );
            const projectCount = (solutionFile.match(/Project\(/g) || [])
                .length;
            ...
            const status = {
                ...
                structure: {
                    core: await fs
                        .access(path.join(PROJECT_ROOT, "BusBuddy.Core"))
                        .then(() => "✅")
                        .catch(() => "❌"),
                    wpf: await fs
                        .access(path.join(PROJECT_ROOT, "BusBuddy.WPF"))
                        .then(() => "✅")
                        .catch(() => "❌"),
                    tests: await fs
                        .access(path.join(PROJECT_ROOT, "BusBuddy.Tests"))
                        .then(() => "✅")
                        .catch(() => "❌"),
                    mcp: await fs
                        .access(path.join(PROJECT_ROOT, "mcp"))
                        .then(() => "✅")
                        .catch(() => "❌"),
                    docs: await fs
                        .access(path.join(PROJECT_ROOT, "Documentation"))
                        .then(() => "✅")
                        .catch(() => "❌"),
                },
            };
...
- Run Tests: bb-test`,
    ...
    async checkSyncfusionStatus() {
        try {
            const result = await this.runPowerShellCommand(
                "if ($env:SYNCFUSION_LICENSE_KEY) { 'License: ✅ Configured' } else { 'License: ❌ Not set' }",
            );

            // Check for Syncfusion references in csproj files
            const wpfProject = await fs.readFile(
                path.join(PROJECT_ROOT, "BusBuddy.WPF", "BusBuddy.WPF.csproj"),
                "utf-8",
            );
            const syncfusionRefs = (wpfProject.match(/Syncfusion\./g) || [])
                .length;
